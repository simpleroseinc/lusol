...
MYBOOL ctf_read_A(char* filename, int maxm, int maxn, int maxnz, int* m, int* n, int* nnzero, int* iA, int* jA,
                  REAL* Aij)
{
    FILE* iofile;
    int eof;
...
MYBOOL ctf_size_A(char* filename, int* m, int* n, int* nnzero)
{
    ...
    return (ctf_read_A(filename, maxint, maxint, maxint, m, n, nnzero, NULL, NULL, NULL));
}
...
MYBOOL mmf_read_A(char* filename, int maxM, int maxN, int maxnz, int* M, int* N, int* nz, int* iA, int* jA, REAL* Aij)
{
    MM_typecode matcode;
    FILE* f;
...
MYBOOL mmf_size_A(char* filename, int* M, int* N, int* nz)
{
    ...
    return (mmf_read_A(filename, maxint, maxint, maxint, M, N, nz, NULL, NULL, NULL));
}
...
MYBOOL mmf_read_b(char* filename, int m, REAL* b)
{
    ...
    return (mmf_read_A(filename, m, 1, m, &im, &jn, &nnzero, NULL, NULL, b));
}

/* Utility routines to read matrix files in Harwell-Boeing format*/

MYBOOL hbf_read_A(char* filename, int maxM, int maxN, int maxnz, int* M, int* N, int* nz, int* iA, int* jA, REAL* Aij)
{
    MYBOOL success;
