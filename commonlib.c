...
void hpsortex(void* attributes, int count, int offset, int recsize, MYBOOL descending, findCompare_func findCompare,
              int* tags)
{
    if (count < 2)
        return;
...
MYBOOL QS_validate(QSORTrec a[], int count)
{
    ...
    {
        iserror = (a[count].prev != a[count - 1].self) || (a[count - 1].next != a[count].self);
        count--;
    }
    if (iserror)
...
double timeNow(void)
{
    ...
    struct timeval t;
    gettimeofday(&t, 0);
    double timesec = (double)t.tv_sec + (double)t.tv_usec / 1000000.0;
    return (timesec);
#endif
}
