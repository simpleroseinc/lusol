...
int readHB_info(const char* filename, int* M, int* N, int* nz, char** Type, int* Nrhs)
{
    ...
    readHB_header(in_file, Title, Key, mat_type, &Nrow, &Ncol, &Nnzero, Nrhs, &Nrhsix, Ptrfmt, Indfmt, Valfmt, Rhsfmt,
...
int readHB_header(FILE* in_file, char* Title, char* Key, char* Type, int* Nrow, int* Ncol, int* Nnzero, int* Nrhs,
                  int* Nrhsix, char* Ptrfmt, char* Indfmt, char* Valfmt, char* Rhsfmt, int* Ptrcrd, int* Indcrd,
                  int* Valcrd, int* Rhscrd, char* Rhstype)
{
    ...
    *(Ptrfmt + 16) = 0;  // (char)NULL;
    *(Indfmt + 16) = 0;  // (char)NULL;
    *(Valfmt + 20) = 0;  // (char)NULL;
    *(Rhsfmt + 20) = 0;  // (char)NULL;
...
int readHB_mat_double(const char* filename, int colptr[], int rowind[], double val[])
{
    ...
    readHB_header(in_file, Title, Key, Type, &Nrow, &Ncol, &Nnzero, &Nrhs, &Nrhsix, Ptrfmt, Indfmt, Valfmt, Rhsfmt,
                  &Ptrcrd, &Indcrd, &Valcrd, &Rhscrd, Rhstype);
    ...
    *(ThisElement + Ptrwidth) = 0;  // (char)NULL;
    count = 0;
    for (i = 0; i < Ptrcrd; i++)
    {
        fgets(line, BUFSIZ, in_file);
    ...
    if (ThisElement == NULL)
        IOHBTerminate("Insufficient memory for ThisElement.");
    *(ThisElement + Indwidth) = 0;  // (char)NULL;
    count = 0;
    for (i = 0; i < Indcrd; i++)
    {
        fgets(line, BUFSIZ, in_file);
    ...
    { /* Skip if pattern only  */
        ...
        if (ThisElement == NULL)
            IOHBTerminate("Insufficient memory for ThisElement.");
        *(ThisElement + Valwidth) = 0;  // (char)NULL;
        count = 0;
        for (i = 0; i < Valcrd; i++)
        {
            fgets(line, BUFSIZ, in_file);
...
int readHB_aux_double(const char* filename, const char AuxType, double b[])
{
    ...
    if (ThisElement == NULL)
        IOHBTerminate("Insufficient memory for ThisElement.");
    *(ThisElement + Rhswidth) = 0;  // (char)NULL;
...
int writeHB_mat_double(const char* filename, int M, int N, int nz, const int colptr[], const int rowind[],
                       const double val[], int Nrhs, const double rhs[], const double guess[], const double exact[],
                       const char* Title, const char* Key, const char* Type, char* Ptrfmt, char* Indfmt, char* Valfmt,
                       char* Rhsfmt, const char* Rhstype)
{
    /****************************************************************************/
...
int readHB_newmat_char(const char* filename, int* M, int* N, int* nonzeros, int** colptr, int** rowind, char** val,
                       char** Valfmt)
{
    FILE* in_file;
    int Nrhs, Nrhsix;
    ...
    *Valfmt = (char*)malloc(21 * sizeof(char));
    ...
    readHB_header(in_file, Title, Key, Type, M, N, nonzeros, &Nrhs, &Nrhsix, Ptrfmt, Indfmt, (*Valfmt), Rhsfmt,
                  &Ptrcrd, &Indcrd, &Valcrd, &Rhscrd, Rhstype);
    fclose(in_file);
    ParseRfmt(*Valfmt, &Valperline, &Valwidth, &Valprec, &Valflag);
...
int writeHB_mat_char(const char* filename, int M, int N, int nz, const int colptr[], const int rowind[],
                     const char val[], int Nrhs, const char rhs[], const char guess[], const char exact[],
                     const char* Title, const char* Key, const char* Type, char* Ptrfmt, char* Indfmt, char* Valfmt,
                     char* Rhsfmt, const char* Rhstype)
{
    /****************************************************************************/
...
int ParseRfmt(char* fmt, int* perline, int* width, int* prec, int* flag)
{
    ...
    {                             /* affects output only, not input */
        if (strchr(fmt, '(') != NULL)
        {
            ...
            len = tmp - tmp3;
            tmp2 = tmp3;
            while (*(tmp2 + len) != 0)  // (int)NULL)
            {
                *tmp2 = *(tmp2 + len);
                tmp2++;
            }
            *(strchr(fmt, ')') + 1) = 0;  //(int)NULL;
        }
    }
