...
void* create_ctx(int pivot_model)
{
    // Usually default pivot_model is LUSOL_PIVMOD_TPP
    LUSOLrec* LUSOL = LUSOL_create(stdout, 0, pivot_model, 0);
    LUSOL->luparm[LUSOL_IP_USEROWL0] = LUSOL_OTHERORDER;
    LUSOL->luparm[LUSOL_IP_SCALAR_NZA] = 10;
    return (LUSOL);
}

void assign_matrix(void* ctx, int* row_indices, int* col_indices, double* values, int nnz)
{
    LUSOLrec* LUSOL = (LUSOLrec*)ctx;
    MYBOOL ok = LUSOL_assign(LUSOL, row_indices, col_indices, values, nnz, TRUE);
    if (!ok)
        printf("LUSOL:: assign matrix failed!!\n");
    return;
}

void factorize_matrix(void* ctx)
{
    LUSOLrec* LUSOL = (LUSOLrec*)ctx;
    int inform = LUSOL_factorize(LUSOL);
    if (inform > LUSOL_INFORM_SERIOUS)
        printf("Error:\n%s\n", LUSOL_informstr(LUSOL, inform));
    return;
}

void col_solve(void* ctx, double* col)
{
    LUSOLrec* LUSOL = (LUSOLrec*)ctx;
    int inform = LUSOL_ftran(LUSOL, col, NULL, FALSE);
    if (inform > LUSOL_INFORM_SERIOUS)
        printf("Error:\n%s\n", LUSOL_informstr(LUSOL, inform));
    return;
}

void row_solve(void* ctx, double* row)
{
    LUSOLrec* LUSOL = (LUSOLrec*)ctx;
    int inform = LUSOL_btran(LUSOL, row, NULL);
    if (inform > LUSOL_INFORM_SERIOUS)
        printf("Error:\n%s\n", LUSOL_informstr(LUSOL, inform));
    return;
}
